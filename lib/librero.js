...
function generarDespiece(paramsUsuario = {}) {
  ...
  const { secciones, anchoTotal } = calcularSecciones(p);
...
// ancho interior puro. El ancho total del librero no es un dato que mande
// el cliente: se calcula acá como la suma real de las secciones (ver
// anchoTotal en el resultado). Con 1 sola sección no se agrega ningún
// divisor.
function calcularSecciones(p) {
  ...
  return { secciones, anchoTotal: x + e };
}
...
function piezasRepisasSeccion(p, seccion, indiceSeccion) {
  ...
  for (let i = 0; i < nRepisas; i++) {
    ...
    piezas.push({
      ...
      // traseros (ver piezasTravesanosTraseros).
      id: `s${indiceSeccion + 1}_repisa_${i + 1}`,
      ancho: anchoSeccion - 4, alto: p.P - 20 - e, espesor: p.e,
      cantos: ['delantero'],
      cantidad: 1,
      color: p.colorInterior, cara: 'interior',
      posicion: { x: xInicio + 2, y, z: e },
      rotacion: 'horizontal',
    });
  }
  return piezas;
