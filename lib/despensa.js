 ...
 *   - Además de piso y techo, cada sección lleva travesaños de refuerzo
 *     (delantero, trasero superior y trasero inferior — y uno trasero
 *     intermedio si el mueble mide 1,4m de alto o más) para que un cuerpo
 *     tan alto no se destranque con el uso.
 *
...
// ---------- 4b. Travesaños de refuerzo por sección (delantero + trasero) ----------
// Piso y techo cierran el cuerpo arriba y abajo, pero en un mueble tan alto
// y angosto como una despensa (hasta 2-3m) esas dos tablas solas no evitan
// que el cuerpo se destranque (racking) con el uso de las puertas o el peso
// de las repisas cargadas. Se agrega, por cada sección, un travesaño justo
// por debajo del techo (delantero y trasero) y uno justo por encima del piso
// (trasero) — mismo criterio estructural que mueble cocina/vanitorio. Van
// por sección (no de lateral a lateral completo) porque cada divisor
// interno ya ocupa ese mismo rango de altura, de piso a techo — mismo
// motivo por el que las repisas también van por sección.
...
function piezasTraviesasSeccion(p, seccion, indiceSeccion) {
  const { e, colorInterior, colorExterior } = p;
  ...
  const piezas = [
    {
      ...
      id: `${prefijo}traviesa_delantera`,
      ancho: anchoSeccion, alto, espesor: e,
      cantos: ['inferior'],
      cantidad: 1,
      color: colorExterior, cara: 'exterior',
      posicion: { x: xInicio, y: H - e - alto, z: P - e },
      rotacion: 'vertical_frontal',
    },
    {
...
function piezasPuertasCorrederas(p) {
  ...
  for (let i = 0; i < nP; i++) {
    ...
    piezas.push({
      ...
      cantidad: 1,
      color: colorExterior, cara: 'exterior',
      posicion: { x: i * paso, y: 8, z: enRielTrasero ? profundidadCuerpo(p) : profundidadCuerpo(p) + 14 },
      rotacion: 'vertical_frontal',
    });
  }
  return piezas;
...
function generarHerrajes(p, { totalRepisas, secciones }) {
  ...
  // Cada travesaño de refuerzo (delantero + trasero superior + trasero
  // inferior, y el intermedio en muebles altos) va atornillado por sus dos
  // extremos (a un lateral o a un divisor), 2 tornillos por extremo.
  const traviesasPorSeccion = p.H >= ALTO_MINIMO_TRAVIESA_INTERMEDIA ? 4 : 3;
  const tornillosTraviesas = secciones.length * traviesasPorSeccion * 4;

  herrajes.push({ tipo: 'tornillo_confirmat', cantidad: 8 + (secciones.length - 1) * 4 + tornillosTraviesas });
  herrajes.push({ tipo: 'tornillo_1_5/8', cantidad: 8 + tornillosTraviesas });
