...
const DEFAULTS = {
  ...
  gp: 18,        // espesor de puertas/frentes
  ...
  if (nC > 0) {
    ...
    for (let i = 0; i < nC; i++) {
      ...
      piezas.push({
        ...
        ancho: anchoSeccion - 4, alto: ALTURA_CAJON_UNIDAD, espesor: p.gp,
        cantos: ['todos'],
        cantidad: 1,
...
function piezasPuertasBatientes(p) {
  const { A, H, nP, gp, colorExterior } = p;
  ...
  for (let i = 0; i < nP; i++) {
    piezas.push({
      ...
      posicion: { x: 2 + i * (anchoPuerta + 2), y: 2, z: -gp },
      rotacion: 'vertical_frontal',
    });
  }
  return piezas;
...
function piezasPuertasCorrederas(p) {
  ...
  for (let i = 0; i < nP; i++) {
    ...
    piezas.push({
      ...
      ancho: anchoPuerta, alto: H - 15, espesor: gp,
      cantos: ['todos'],
      cantidad: 1,
      color: colorExterior, cara: 'exterior',
      posicion: { x: i * paso, y: 8, z: enRielTrasero ? -gp - 14 : -gp },
      rotacion: 'vertical_frontal',
    });
  }
  return piezas;
...
function generarHerrajes(p, { totalCajones, totalRepisas, secciones, divisores }) {
  ...
  if (totalCajones > 0) {
    ...
    herrajes.push({ tipo: 'tirador_o_sistema_push', cantidad: totalCajones });
  }
  ...
  if (p.nP > 0) {
    if (p.tipoPuerta === 'corredera') {
      ...
      herrajes.push({ tipo: 'tirador_o_sistema_push', cantidad: p.nP });
    }
  }
