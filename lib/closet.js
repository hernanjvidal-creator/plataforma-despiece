 ...
 * (solo necesitan un riel, no un panel real detrás de cada borde). Las
 * puertas batientes, en cambio, SÍ dependen de las secciones: cada bisagra
 * necesita un panel real (lateral o divisor) detrás de su canto, así que
 * cada sección (o grupo de secciones unidas por un borde "abierto", sin
 * divisor real entre sí) arma sus propias puertas dentro de su propio
 * ancho — una, o dos si el ancho supera ANCHO_MAXIMO_PUERTA (ver
 * piezasPuertasBatientes). Nunca se reparten a ciegas sobre el ancho total
 * como antes.
 *
 * Travesaños traseros (superior, inferior y uno intermedio en muebles
 * altos): van de lateral a lateral completo, por detrás de TODAS las
 * secciones (a diferencia de despensa.js, donde van por sección) — un
 * closet puede tener muchas secciones, y cortar un travesaño en cada una
 * sería un enredo de piezas. Para que puedan pasar limpios por detrás sin
 * chocar con los divisores internos, cada divisor queda recortado 15mm de
 * profundidad (ver piezasDivisores) — su borde trasero no llega hasta el
 * fondo, dejando ese espacio libre para el travesaño.
 *
...
const ALTO_MINIMO_TRAVIESA_INTERMEDIA = 1400; // mm, alto del mueble

// Refuerzo trasero de lateral a lateral (ver comentario de cabecera). El
// intermedio solo se agrega en muebles altos, donde el tramo entre el
// travesaño superior y el inferior queda demasiado largo sin apoyo.
function piezasTravesanosTraseros(p) {
  ...
  if (p.H >= ALTO_MINIMO_TRAVIESA_INTERMEDIA) {
    piezas.push({
      id: 'travesano_trasero_intermedio',
      ancho: A - 2 * e, alto, espesor: e,
      cantos: [],
      cantidad: 1,
      color: colorInterior, cara: 'interior',
      posicion: { x: e, y: (H - alto) / 2, z: 0 },
      rotacion: 'vertical_frontal',
    });
  }
...
// Tampoco llega hasta el fondo (z=0): queda recortado 15mm de profundidad
// para que los travesaños traseros (de lateral a lateral, ver
// piezasTravesanosTraseros) puedan pasar limpios por detrás sin chocar con
// él — mismo criterio que con el piso/techo, pero en el eje de profundidad.
function piezasDivisores(p, divisores) {
  const { H, e, colorInterior } = p;
  const P = profundidadCuerpo(p);
  return divisores.map(d => ({
    id: d.id,
    ancho: P - e, alto: H - 2 * e, espesor: e,
    cantos: [],
    cantidad: 1,
    color: colorInterior, cara: 'interior',
...
function piezasPuertasBatientes(p, secciones) {
  ...
  grupos.forEach(grupo => {
    ...
    // Si alguno de los dos bordes del grupo es un divisor real (no un
    // lateral exterior), la puerta de ese lado comparte el separador con la
    // puerta del grupo vecino — necesita bisagra codo semicurva en vez de
    // la recta, porque el separador (15/18mm) no tiene espesor para dos
    // placas de montaje enfrentadas.
    let cantosSemicurva = 0;
    if (grupo.holguraIzq === 1) cantosSemicurva++;
    if (grupo.holguraDer === 1) cantosSemicurva++;
    totalPuertasSemicurva += Math.min(cantosSemicurva, nPuertas);
  });
...
function generarHerrajes(p, {
  ...
  // Cada travesaño trasero va atornillado en sus dos extremos (2 tornillos
  // por lado) y, al pasar por detrás de cada divisor interno, también ahí
  // (2 tornillos por cruce).
  const numTravesanos = p.H >= ALTO_MINIMO_TRAVIESA_INTERMEDIA ? 3 : 2;
  const tornillosTravesanos = numTravesanos * (4 + divisores.length * 2);

  // cuerpo + fijación de divisores + tapa de cada cajonera + travesaños traseros + cajas de cajón
  herrajes.push({ tipo: 'tornillo_confirmat / tornillo_1_5/8', cantidad: 8 + divisores.length * 4 + totalCajoneras * 4 + tornillosTravesanos + totalCajones * 4 });
