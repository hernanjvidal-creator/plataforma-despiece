...
const DEFAULTS = {
  ...
  nC: 3,         // cantidad de cajones en la cajonera
  correderaTipo: 'bola',
  cubierta: {
...
function validarParametros(p) {
  ...
  if (p.nC < 1) throw new Error('La cajonera debe tener al menos 1 cajón');
...
// ---------- 3. Frentes de cajón (apilados, ocupan toda la cajonera) ----------
function piezasFrentesCajonera(p, xInicio) {
  const { anchoCajonera, H, nC, e, colorExterior } = p;
...
function generarHerrajes(p) {
  ...
  herrajes.push({ tipo: 'tarugo_minifix', cantidad: 8 + p.nC * 4 });
