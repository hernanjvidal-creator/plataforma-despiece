/**
 * MOTOR DE REGLAS — Mueble Esquinero Interior (Bajo Cocina)
 * -----------------------------------------------------------
 * Dos "brazos" rectangulares (cuerpos de mueble bajo estándar) que se
 * encuentran en 90° en una esquina, compartiendo el mismo piso/pared.
 * El Brazo A corre a lo largo de la Pared 1 (eje X global). El Brazo B
 * corre a lo largo de la Pared 2 (eje Z global), perpendicular al A.
 *
 * Como todo el resto de la app trabaja en un solo plano (x=ancho,
 * z=profundidad), el Brazo B se genera primero en SU PROPIO plano local
 * (igual que un mueble lineal normal) y después se rota 90° hacia el
 * espacio global: alto/ancho locales que corrían en X pasan a correr en Z
 * y viceversa (ver `rotarPiezaBrazoB`). Es una rotación en múltiplos de
 * 90°, así que basta con permutar ejes — no hace falta trigonometría.
 *
 * "Esquinero interior": la esquina estructural (donde se juntan las dos
 * paredes) queda escondida detrás del mueble; el lado accesible es el dos
 * puertas que llegan hasta la esquina, una por brazo, encontrándose ahí —
 * en la práctica se resuelve con un herraje de puerta plegable de rincón
 * (no incluido como pieza, es hardware de comercio). Cada brazo se puede
 * empalmar con otro mueble bajo estándar en su extremo exterior.
 *
 * Alto (H): es la altura TOTAL desde el piso hasta la parte de arriba del
 * cuerpo — incluye el zócalo estándar de 100mm. El cuerpo de cada brazo
 * (laterales, puertas) mide H - 100mm.
 *
 * Sistema de coordenadas global (mm), igual que el resto de los módulos:
 *   x: distancia a lo largo de la Pared 1 (0 = esquina)
 *   y: altura (0 = piso del cuerpo, sin contar zócalo)
 *   z: distancia a lo largo de la Pared 2 (0 = esquina)
 */

import { resumirPlanchas } from './shared';

const DEFAULTS = {
  anchoA: 900,   // largo del Brazo A medido desde la esquina
  anchoB: 900,   // largo del Brazo B medido desde la esquina
  H: 700,        // alto TOTAL desde el piso (incluye el zócalo de 100mm)
  P: 560,        // profundidad de cada brazo
  e: 15,         // espesor tablero estructural
  espesorPuertas: 15, // espesor de las puertas — 15 (estándar) o 18 (opcional)
  hp: 100,       // altura de zócalo (estándar, no editable por el cliente)
  r: 50,         // retranqueo zócalo
  colorInterior: 'blanco',
  colorExterior: 'blanco',
};

// Altura real del cuerpo de cada brazo, descontando el zócalo de la altura
// TOTAL que ingresa el cliente.
function alturaCuerpo(p) {
  return p.H - p.hp;
}

function generarDespiece(paramsUsuario = {}) {
  const p = { ...DEFAULTS, ...paramsUsuario };
  validarParametros(p);

  // Brazo A: se genera directo en el plano global (sin transformar).
  const piezasA = [
    ...generarCuerpoBrazo('a', p.anchoA, p),
    ...generarRespaldoBrazo('a', p.anchoA, p),
    ...generarZocaloBrazo('a', p.anchoA, p),
    ...generarPuertasBrazo('a', p.anchoA, p),
  ];

  // Brazo B: se genera en su plano local (como un mueble lineal normal) y
  // se rota 90° hacia el espacio global, calzando su origen con la esquina.
  const piezasBLocal = [
    ...generarCuerpoBrazo('b', p.anchoB, p),
    ...generarRespaldoBrazo('b', p.anchoB, p),
    ...generarZocaloBrazo('b', p.anchoB, p),
    ...generarPuertasBrazo('b', p.anchoB, p),
  ];
  const piezasB = piezasBLocal.map(pieza => rotarPiezaBrazoB(pieza));

  const piezas = [...piezasA, ...piezasB];
  const herrajes = generarHerrajes(p);
  const notas = generarNotas(p);

  return {
    modulo: 'esquinero_bajo_cocina',
    parametros: p,
    piezas,
    herrajes,
    notas,
    resumen: resumirPlanchas(piezas),
  };
}

// ---------- Validación básica ----------
function validarParametros(p) {
  if (p.anchoA < 300 || p.anchoA > 3000) throw new Error('Ancho del brazo A fuera de rango 300-3000mm');
  if (p.anchoB < 300 || p.anchoB > 3000) throw new Error('Ancho del brazo B fuera de rango 300-3000mm');
  if (p.H < 50 || p.H > 3000) throw new Error('Alto (H) fuera de rango 50-3000mm');
  if (p.H <= p.hp) {
    throw new Error(`El alto (H) debe ser mayor que el zócalo estándar (${p.hp}mm) — sube el alto total.`);
  }
  if (p.P < 300 || p.P > 1000) throw new Error('Profundidad (P) fuera de rango 300-1000mm');
}

// ---------- 1. Cuerpo de un brazo (igual patrón que muebleBajoCocina, parametrizado) ----------
function generarCuerpoBrazo(prefijo, ancho, p) {
  const { P, e, colorInterior } = p;
  const H = alturaCuerpo(p);
  return [
    {
      id: `${prefijo}_lateral_izq`,
      ancho: P, alto: H, espesor: e,
      cantos: ['delantero'], cantidad: 1,
      color: colorInterior, cara: 'interior',
      posicion: { x: 0, y: 0, z: 0 },
      rotacion: 'vertical_profundidad',
    },
    {
      id: `${prefijo}_lateral_der`,
      ancho: P, alto: H, espesor: e,
      cantos: ['delantero'], cantidad: 1,
      color: colorInterior, cara: 'interior',
      posicion: { x: ancho - e, y: 0, z: 0 },
      rotacion: 'vertical_profundidad',
    },
    {
      id: `${prefijo}_piso`,
      ancho: ancho - 2 * e, alto: P - 20, espesor: e,
      cantos: [], cantidad: 1,
      color: colorInterior, cara: 'interior',
      posicion: { x: e, y: 0, z: 0 },
      rotacion: 'horizontal',
    },
    {
      id: `${prefijo}_traviesa_delantera`,
      ancho: ancho - 2 * e, alto: 100, espesor: e,
      cantos: ['inferior'], cantidad: 1,
      color: colorInterior, cara: 'interior',
      posicion: { x: e, y: H - e, z: P - 100 },
      rotacion: 'horizontal',
    },
    {
      id: `${prefijo}_traviesa_trasera`,
      ancho: ancho - 2 * e, alto: 100, espesor: e,
      cantos: [], cantidad: 1,
      color: colorInterior, cara: 'interior',
      posicion: { x: e, y: H - e, z: 0 },
      rotacion: 'horizontal',
    },
  ];
}

function generarRespaldoBrazo(prefijo, ancho, p) {
  const { e } = p;
  const H = alturaCuerpo(p);
  return [{
    id: `${prefijo}_respaldo`,
    ancho: ancho - 2 * e + 16, alto: H - 30, espesor: 3,
    cantos: [], cantidad: 1,
    material: 'HDF',
    posicion: { x: e - 8, y: 15, z: 0 },
    rotacion: 'vertical_frontal',
  }];
}

// Zócalo del brazo: panel frontal (recesado desde el frente real) + panel
// trasero (contra la pared) + retorno en el extremo exterior (no en el que
// da a la esquina, que queda contra el otro brazo) + cruces de soporte cada
// ESPACIADO_CRUCE_ZOCALO mm — igual patrón que muebleBajoCocina.js.
const ESPACIADO_CRUCE_ZOCALO = 800;

function generarZocaloBrazo(prefijo, ancho, p) {
  const { P, e, hp, r, colorInterior, colorExterior } = p;
  const altoZocalo = hp - 10;
  const profundidadZocalo = P - r;

  const piezas = [
    {
      id: `${prefijo}_zocalo_frontal`,
      ancho, alto: altoZocalo, espesor: e,
      cantos: ['superior'], cantidad: 1,
      color: colorExterior, cara: 'exterior',
      posicion: { x: 0, y: -hp, z: profundidadZocalo - e },
      rotacion: 'vertical_frontal',
    },
    {
      id: `${prefijo}_zocalo_trasero`,
      ancho, alto: altoZocalo, espesor: e,
      cantos: [], cantidad: 1,
      color: colorInterior, cara: 'interior',
      posicion: { x: 0, y: -hp, z: 0 },
      rotacion: 'vertical_frontal',
    },
    {
      id: `${prefijo}_zocalo_lateral_exterior`,
      ancho: profundidadZocalo, alto: altoZocalo, espesor: e,
      cantos: ['delantero'], cantidad: 1,
      color: colorExterior, cara: 'exterior',
      posicion: { x: ancho - e, y: -hp, z: 0 },
      rotacion: 'vertical_profundidad',
    },
  ];

  let x = ESPACIADO_CRUCE_ZOCALO;
  let n = 1;
  while (x < ancho - 200) {
    piezas.push({
      id: `${prefijo}_zocalo_cruce_${n}`,
      ancho: profundidadZocalo, alto: altoZocalo, espesor: e,
      cantos: [], cantidad: 1,
      color: colorInterior, cara: 'interior',
      posicion: { x, y: -hp, z: 0 },
      rotacion: 'vertical_profundidad',
    });
    x += ESPACIADO_CRUCE_ZOCALO;
    n++;
  }

  return piezas;
}

// Puertas del brazo, cubriendo todo su largo — incluido el extremo que,
// tras la rotación, queda pegado a la esquina. Las dos puertas más cercanas
// a la esquina (una por brazo) se encuentran ahí; en obra se resuelven con
// un herraje de puerta plegable de rincón (ver notas de producción).
function generarPuertasBrazo(prefijo, ancho, p) {
  const { P, espesorPuertas, colorExterior } = p;
  const H = alturaCuerpo(p);

  const nP = ancho > 500 ? 2 : 1;
  const anchoUtil = ancho - (nP + 1) * 2;
  const anchoPuerta = anchoUtil / nP;
  const piezas = [];
  for (let i = 0; i < nP; i++) {
    piezas.push({
      id: `${prefijo}_puerta_${i + 1}`,
      ancho: anchoPuerta, alto: H - 4, espesor: espesorPuertas,
      cantos: ['todos'], cantidad: 1,
      color: colorExterior, cara: 'exterior',
      posicion: { x: 2 + i * (anchoPuerta + 2), y: 2, z: P },
      rotacion: 'vertical_frontal',
    });
  }
  return piezas;
}

// ---------- 2. Rotación 90° del Brazo B hacia el espacio global ----------
// vertical_frontal <-> vertical_profundidad se intercambian (ambos ya usan
// el campo "ancho" para un eje horizontal, solo cambia cuál). Las piezas
// horizontales intercambian directamente ancho<->alto. En los tres casos,
// la posición se pasa de (x,y,z) local a (z,y,x) global.
function rotarPiezaBrazoB(pieza) {
  const { x, y, z } = pieza.posicion;

  if (pieza.rotacion === 'horizontal') {
    return {
      ...pieza,
      id: pieza.id,
      ancho: pieza.alto,
      alto: pieza.ancho,
      posicion: { x: z, y, z: x },
    };
  }

  const nuevaRotacion = pieza.rotacion === 'vertical_frontal' ? 'vertical_profundidad' : 'vertical_frontal';
  return {
    ...pieza,
    rotacion: nuevaRotacion,
    posicion: { x: z, y, z: x },
  };
}

// ---------- 3. Herrajes ----------
function generarHerrajes(p) {
  const herrajes = [];

  const nPA = p.anchoA > 500 ? 2 : 1;
  const nPB = p.anchoB > 500 ? 2 : 1;
  const totalPuertas = nPA + nPB;

  const H = alturaCuerpo(p);
  const bisagrasPorPuerta = H > 900 ? 3 : 2;
  herrajes.push({ tipo: 'bisagra_codo', cantidad: totalPuertas * bisagrasPorPuerta });
  herrajes.push({ tipo: 'manilla_puerta_negra_moderna', cantidad: totalPuertas });

  herrajes.push({ tipo: 'bisagra_plegable_rincon', cantidad: 1 });

  const patas = (p.anchoA + p.anchoB) > 1800 ? 8 : 6; // un apoyo extra cerca de la esquina
  herrajes.push({ tipo: 'pata_regulable', cantidad: patas });
  herrajes.push({ tipo: 'tornillo_golfari', cantidad: 16 });
  herrajes.push({ tipo: 'tarugo_minifix', cantidad: 16 });

  return herrajes;
}

// ---------- 4. Notas de producción ----------
function generarNotas(p) {
  const notas = [
    'Las puertas de ambos brazos llegan hasta la esquina y se encuentran ahí: se resuelven con un herraje de puerta plegable de rincón (bisagra especial de dos hojas), no con bisagras rectas comunes — verificar medida exacta del herraje elegido antes de cortar.',
    'Los dos brazos comparten la esquina pero son cuerpos independientes: no llevan un panel de esquina compartido en este modelo — revisar en terreno si se necesita un refuerzo o unión adicional donde se encuentran (incluye el retorno del zócalo en ese punto).',
  ];
  if (p.espesorPuertas !== p.e) {
    notas.push(`Puertas en ${p.espesorPuertas}mm (más gruesas que el resto del cuerpo, en ${p.e}mm) — quedan en un grupo de material aparte para el corte.`);
  }
  return notas;
}

export { generarDespiece };
