...
function piezasCuerpo(p) {
  const { A, H, P, e, colorInterior, colorExterior } = p;
...
// ---------- 3. Repisas interiores (ajustables por soportes metálicos) ----------
...
// arriba de los ojos, la base se ve desde abajo — estos paneles cuelgan
// bajo el cuerpo, en todo el contorno (frente, fondo y los dos costados),
// para que se vea terminado por los cuatro lados y no el hueco crudo del
// piso interior. Mismo patrón que el zócalo de mueble cocina, sin
// retranqueo ni cruces (acá no hay patas que sostener).
...
function piezasZocaloInferior(p) {
  ...
  const alto = ALTO_ZOCALO_INFERIOR;

  return [
    {
      id: 'zocalo_inferior_frontal',
    ...
    {
      id: 'zocalo_inferior_lateral_izq',
    ...
    {
      id: 'zocalo_inferior_lateral_der',
      ancho: P, alto, espesor: e,
      cantos: ['delantero'],
      cantidad: 1,
      color: colorExterior, cara: 'exterior',
...
function generarHerrajes(p) {
  ...
  if (p.nBaldas > 0) {
    herrajes.push({ tipo: 'soporte_repisa_metalico', cantidad: p.nBaldas * 4 });
  }
