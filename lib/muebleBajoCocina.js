...
function calcularSecciones(p) {
  ...
  const anchosInteriores = p.secciones.map((s, i) => {
    ...
    if (interior < 100) {
      throw new Error(
        `Sección ${i + 1}: con ${Math.round(nominal)}mm de ancho fijo, compartiendo separadores con sus vecinas, el hueco interior real quedaría en solo ${Math.round(interior)}mm. Sube el ancho fijo de esta sección a al menos ${Math.round(extIzq + extDer + 100)}mm.`
      );
    }
  ...
  if (anchoRestante < 0) {
    throw new Error(
      `Las secciones de este mueble (sumando sus anchos fijos) no caben dentro de un mueble de ${A}mm de ancho exterior. Sube el "Ancho (mm)" de arriba a al menos ${anchoMinimoA}mm, o quita/achica secciones.`
    );
  }
  ...
  if (nAutomaticas > 0 && anchoAutomatico < ANCHO_MIN_SECCION_AUTOMATICA) {
    throw new Error(
      `Con estas secciones, las de ancho libre quedan de solo ${Math.round(anchoAutomatico)}mm de ancho interior. Sube el "Ancho (mm)" de arriba (ancho exterior del mueble) a al menos ${anchoMinimoA}mm, o reduce/achica secciones.`
    );
  }
...
function generarPuertasLavaplatos(p, seccion, indice) {
  ...
  for (let i = 0; i < nP; i++) {
    piezas.push({
      ...
      ancho: anchoFrente, alto: H - 4, espesor: p.espesorPuertas,
      cantos: ['todos'],
      cantidad: 1,
      color: p.colorExterior, cara: 'exterior',
