...
function generarDespiece(paramsUsuario = {}) {
  ...
  p.secciones.forEach((seccion, i) => {
    ...
    if (seccion.tipo !== 'lavavajillas') {
      accesorios.push(...piezasPatasVisuales(anchoModulo, profundidadModulo(p)).map(pz => trasladarYPrefijar(pz, x, prefijo)));
...
function generarHerrajes(p, { infoPorModulo, totalCajones, totalPuertas, totalRepisas, numModulos, totalModulosConPatas, anchoTotal }) {
  ...
  herrajes.push({
    ...
    cantidad: numModulos * tornillosPorModulo + totalCajones * 2,
  });
