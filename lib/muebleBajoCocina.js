...
// ---------- 2. Respaldo ----------
// En un mueble normal (contra la pared) es MDF, invisible. En una isla, el
...
function piezasRespaldo(p) {
  ...
  if (isla) {
    return [{
      id: 'respaldo',
      ancho: A - 2 * e + 16, alto: H - 2 * e + 16, espesor: e,
      cantos: [],
      cantidad: 1,
      color: colorExterior, cara: 'exterior',
...
function generarHerrajes(p, { secciones, totalCajones, totalPuertas, totalRepisas, puertasPorSeccion }) {
  ...
  // divisor interno (ver piezasDivisores) — 2 tornillos extra por cruce.
  const tornillosCruceDivisores = (secciones.length - 1) * 2 * 2;
  ...
  herrajes.push({ tipo: 'tornillo_1_5/8', cantidad: 8 + (secciones.length - 1) * 4 + 12 + totalCajones * 2 + tornillosCruceDivisores });
...
function generarNotas(p, secciones) {
  ...
  if (p.isla) {
    notas.push('Mueble isla: además de los laterales, el respaldo y el zócalo trasero también van terminados en el color exterior, ya que este mueble queda a la vista por los cuatro costados.');
  }
