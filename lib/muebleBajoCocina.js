...
function generarDespiece(paramsUsuario = {}) {
  ...
  tramos.forEach((tramo, i) => {
    ...
    const seccionesConCiego = [
      ...(i > 0 ? [{ tipo: 'ciego', ancho: p.P }] : []),
      ...tramo.secciones,
      ...(i < tramos.length - 1 ? [{ tipo: 'ciego', ancho: p.P }] : []),
    ];
  ...
  notas.push(
    ...
    'con la del otro brazo. La caja (laterales/piso/zócalo) sí llega hasta el fondo de la esquina. Para aprovechar esa ' +
    'zona ciega se recomienda un accesorio de esquina (bandeja extraíble o esquinero giratorio tipo "lazy susan"), no ' +
    'incluido como pieza de melamina — cotizar aparte según el hueco resultante.'
  );
...
function validarSecciones(secciones) {
  const tiposValidos = ['estandar', 'lavaplatos', 'lavavajillas', 'horno', 'cajones_olleros', 'cajones_cubiertos', 'ciego'];
  for (const s of secciones) {
...
function generarSeccionFrentes(p, seccion, indice) {
  switch (seccion.tipo) {
    case 'ciego':
      return { piezas: [], cajaInfos: [] }; // zona ciega de esquina: sin frente, a propósito
    case 'lavavajillas':
    case 'horno':
