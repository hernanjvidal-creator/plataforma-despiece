...
function generarCajonesOlleros(p, seccion, indice) {
  ...
  if (altoSaldo < MIN_ALTURA_CAJON_SALDO) {
    throw new Error(
      `Módulo ${indice + 1} (cajones olleros): con 2 cajones de ${ALTURA_CAJON_OLLERO}mm abajo, el cajón superior quedaría de solo ${Math.round(altoSaldo)}mm. Sube el alto (H) — necesitas al menos ${Math.round(y3 + gap + MIN_ALTURA_CAJON_SALDO)}mm de alto de cuerpo.`
    );
  }
