...
function generarCajonesOlleros(p, seccion, indice) {
  ...
  // El cajón de arriba (el del saldo) llega a la misma altura que el tope
  // de una puerta — 30mm más abajo que los laterales — igual que en
  // "solo cajones", para que quede a la misma línea visual que las
  // secciones vecinas y no tope con la cubierta.
  const altoSaldo = (H - 30) - y3;

  if (altoSaldo < MIN_ALTURA_CAJON_SALDO) {
    throw new Error(
      `Módulo ${indice + 1} (cajones olleros): con 2 cajones de ${ALTURA_CAJON_OLLERO}mm abajo, el cajón superior quedaría de solo ${Math.round(altoSaldo)}mm. Sube el alto (H) — necesitas al menos ${Math.round(y3 + 30 + MIN_ALTURA_CAJON_SALDO)}mm de alto de cuerpo.`
    );
  }

  const alturas = [ALTURA_CAJON_OLLERO, ALTURA_CAJON_OLLERO, altoSaldo];
  const posicionesY = [y1, y2, y3];
  ...
  alturas.forEach((alto, i) => {
    ...
    piezas.push({
      ...
      ancho: anchoFrente, alto, espesor: p.e,
      cantos: ['todos'],
      cantidad: 1,
      color: p.colorExterior, cara: 'exterior',
...
function tipoFrenteSeccion(seccion) {
  ...
  if (seccion.tipo === 'cajones_olleros') return { puerta: false, cajon: true };
...
function generarHerrajes(p, { secciones, totalCajones, totalPuertas, totalRepisas, puertasPorSeccion }) {
  ...
  herrajes.push({ tipo: 'tornillo_1_5_8', cantidad: 8 + (secciones.length - 1) * 4 + 12 + totalCajones * 2 });
