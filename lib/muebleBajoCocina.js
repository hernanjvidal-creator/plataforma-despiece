...
function generarDespiece(paramsUsuario = {}) {
  ...
  tramos.forEach((tramo, i) => {
    const anchoTramo = calcularAnchoTramo(tramo.secciones, p.e);
    ...
    const r = generarTramo(p, anchoTramo, tramo.secciones, prefijo, heading, origenX, origenZ);
  ...
  herrajes.push({ tipo: 'bisagra_plegable_rincon', cantidad: tramos.length - 1 });
  ...
  notas.push(
    `Este mueble tiene ${tramos.length - 1} esquina(s) ("esquinero interior"): las puertas de los dos brazos que se ` +
    'encuentran en cada una se resuelven con un herraje de puerta plegable de rincón, no con bisagras rectas comunes. ' +
    'Los brazos no comparten un panel de esquina en este modelo — revisar en terreno si se necesita un refuerzo ' +
    'adicional donde se encuentran (incluye el retorno del zócalo en ese punto).'
  );
