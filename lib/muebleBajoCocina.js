...
function calcularSecciones(p) {
  ...
  if (anchoRestante < 0) {
    throw new Error(
      `Las secciones con ancho fijo (${Math.round(anchoFijoTotal)}mm en total, medida interior) no caben dentro de un mueble de ${A}mm de ancho exterior: los 2 laterales (${e}mm cada uno) ya restan ${2 * e}mm de espacio interior. Sube el "Ancho (mm)" de arriba a al menos ${anchoMinimoA}mm, o quita/achica secciones.`
    );
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    const xInicio = x;
    const bordeIzq = bordes[i];
    const bordeDer = bordes[i + 1];
    const extIzq = extensionPorBorde(bordeIzq);
    const extDer = extensionPorBorde(bordeDer);
    secciones.push({
      ...p.secciones[i],
      xInicio, anchoSeccion,
      xInicioFrente: xInicio - extIzq,
      anchoSeccionFrente: anchoSeccion + extIzq + extDer,
      holguraIzq: holguraPorBorde(bordeIzq),
      holguraDer: holguraPorBorde(bordeDer),
    });
    x += anchoSeccion;
    if (i < n - 1) {
