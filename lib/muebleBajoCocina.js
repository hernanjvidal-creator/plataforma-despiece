...
function validarParametros(p) {
  ...
  for (const s of p.secciones) {
    ...
    if (s.tipo === 'estandar') {
      ...
      if (nP === 0 && nC === 0) throw new Error('Una sección "estandar" debe tener al menos 1 puerta o 1 cajón');
...
function piezasRespaldo(p) {
  const { A, H, e, isla, colorExterior } = p;
  if (isla) {
    return [{
      id: 'respaldo',
...
function piezasZocalo(p) {
  ...
  return [{
    id: 'panel_zocalo',
    ancho: A, alto: hp - 10, espesor: p.e,
    cantos: ['superior'],
    cantidad: 1,
    color: p.colorExterior, cara: 'exterior',
    posicion: { x: 0, y: -hp, z: p.r },
    rotacion: 'vertical_frontal',
  }];
}
...
function generarPuertasLavaplatos(p, seccion, indice) {
  ...
  for (let i = 0; i < nP; i++) {
    piezas.push({
      ...
      ancho: anchoPuerta, alto: p.H - 4, espesor: p.gp,
      cantos: ['todos'],
      cantidad: 1,
      color: p.colorExterior, cara: 'exterior',
      posicion: { x: xInicio + 2 + i * (anchoPuerta + 2), y: 2, z: -p.gp },
      rotacion: 'vertical_frontal',
    });
  }
...
function accesorioLavaplatos(p, seccion, indice) {
  ...
  return [{
    ...
    posicion: {
      ...
      y: p.H + espesorCubierta - alturaCubeta,
...
function generarCajonesApilados(p, seccion, indice, alturaUnidad) {
  ...
  for (let i = 0; i < nCajones; i++) {
    piezas.push({
      ...
      ancho: anchoSeccion - 4, alto: alturaUnidad, espesor: p.gp,
      cantos: ['todos'],
      cantidad: 1,
      color: p.colorExterior, cara: 'exterior',
      posicion: { x: xInicio + 2, y, z: -p.gp },
      rotacion: 'vertical_frontal',
    });
  ...
  if (y - gap > p.H - 6) {
    throw new Error(
      `Sección ${indice + 1}: ${nCajones} cajón(es) de ${alturaUnidad}mm no caben en el alto (H=${p.H}mm). Reduce la cantidad o aumenta H.`
    );
  }
  return { piezas, cajaInfos };
}
...
function generarFrenteEstandar(p, seccion, indice) {
  ...
  if (config === 'solo_puertas') {
    ...
    piezas.push(...piezasRepisasZona(p, seccion, indice, p.e, p.H - p.e));
  }

  if (config === 'solo_cajones') {
    const alturaUtil = p.H - (nC + 1) * 3;
    const alturaCajon = alturaUtil / nC;
    for (let i = 0; i < nC; i++) {
      const y = 3 + i * (alturaCajon + 3);
      piezas.push({
        id: `s${indice + 1}_frente_cajon_${i + 1}`,
        ancho: anchoSeccion - 4, alto: alturaCajon, espesor: p.gp,
        cantos: ['todos'],
        cantidad: 1,
        color: p.colorExterior, cara: 'exterior',
        posicion: { x: xInicio + 2, y, z: -p.gp },
  ...
  if (config === 'mixto') {
    ...
    let ySiguiente = p.H - gap;
    for (let i = 0; i < nCajones; i++) {
      const y = ySiguiente - alturaCajonUnidad;
      piezas.push({
        id: `s${indice + 1}_frente_cajon_${i + 1}`,
        ancho: anchoSeccion - 4, alto: alturaCajonUnidad, espesor: p.gp,
        cantos: ['todos'],
        cantidad: 1,
        color: p.colorExterior, cara: 'exterior',
        posicion: { x: xInicio + 2, y, z: -p.gp },
    ...
    for (let i = 0; i < nP; i++) {
      piezas.push({
        id: `s${indice + 1}_puerta_${i + 1}`,
        ancho: anchoPuerta, alto: alturaPuertas, espesor: p.gp,
        cantos: ['todos'],
        cantidad: 1,
        color: p.colorExterior, cara: 'exterior',
        posicion: { x: xInicio + 2 + i * (anchoPuerta + 2), y: gap, z: -p.gp },
        rotacion: 'vertical_frontal',
      });
    }
...
function generarHerrajes(p, { secciones, totalCajones, totalPuertas, totalRepisas }) {
  ...
  if (totalPuertas + totalCajones > 0) {
    herrajes.push({ tipo: 'tirador_o_sistema_push', cantidad: totalPuertas + totalCajones });
  }
