...
// PE cubre también Colombia y Ecuador: los tres mercados comparten el mismo
// formato grande de 2.15x2.44m (marcas como Pelikano/Madecentro operan en
// los tres países con ese estándar), además del formato chico 1220x2440
// (igual que MX/US) que también se consigue ahí.
const PLANCHAS_ESTANDAR = {
  ...
  PE: { nombre: 'Perú/Colombia/Ecuador — Melamina 2150x2440', ancho: 2150, alto: 2440 },
 ...
 *
 * Se corre el empaquetado completo dos veces, con dos reglas de corte
 * distintas (ver colocarPieza), y se usa el resultado que necesite menos
 * planchas. Ninguna de las dos reglas es mejor en todos los casos (es un
 * heurístico greedy, no hay una única forma óptima de partir el sobrante en
 * cada paso) — probar ambas y quedarse con la mejor evita que el cambio de
 * regla empeore el resultado en algún mueble puntual, y captura la mejora
 * cuando corresponde.
 */
function nestearGrupo(piezas, anchoUtil, altoUtil, kerf, plancha) {
  const resultadoFijo = ejecutarNesting(piezas, anchoUtil, altoUtil, kerf, false);
  const resultadoDinamico = ejecutarNesting(piezas, anchoUtil, altoUtil, kerf, true);
  const planchas = resultadoDinamico.length < resultadoFijo.length ? resultadoDinamico : resultadoFijo;
...
function ejecutarNesting(piezas, anchoUtil, altoUtil, kerf, splitDinamico) {
  ...
  for (const pieza of piezasOrdenadas) {
    ...
    for (const p of planchas) {
      ...
      if (espacio) {
        colocarPieza(p, espacio, pieza, kerf, splitDinamico);
        colocada = true;
        break;
      }
    }
    ...
    if (!colocada) {
      ...
      colocarPieza(nuevaPlancha, espacio, pieza, kerf, splitDinamico);
...
function colocarPieza(plancha, match, pieza, kerf, splitDinamico) {
  ...
  // Hay dos formas válidas de partir en dos lo que sobra del espacio libre
  // (ninguna pierde área): dejar fijo "el de la derecha se queda con el
  // alto de la pieza" (splitDinamico=false) puede dibujar, con una pieza
  // baja y ancha (ej. un piso), un corredor angosto al lado que después
  // ninguna otra pieza logra ocupar (queda cortado justo al ras, y el kerf
  // hace que ni las piezas del mismo alto quepan) — se pierde esa plancha
  // completa aunque sobre espacio de sobra. La variante dinámica evita eso
  // comparando qué tan "angosta" queda cada alternativa, pero no es
  // estrictamente mejor en todos los casos (nestearGrupo corre ambas y se
  // queda con la que use menos planchas).
  plancha.espaciosLibres = plancha.espaciosLibres.filter(e => e !== espacio);

  let espacioDerecha, espacioAbajo;
  if (splitDinamico && alto * espacio.ancho < ancho * espacio.alto) {
    // La pieza es proporcionalmente más "baja" que "angosta": conviene que
    // el rectángulo de la derecha se quede con el alto completo (en vez de
    // acotado al alto de la pieza), y el de abajo con el ancho de la pieza.
    espacioDerecha = { x: espacio.x + ancho, y: espacio.y, ancho: espacio.ancho - ancho, alto: espacio.alto };
    espacioAbajo = { x: espacio.x, y: espacio.y + alto, ancho: ancho, alto: espacio.alto - alto };
  } else {
    espacioDerecha = { x: espacio.x + ancho, y: espacio.y, ancho: espacio.ancho - ancho, alto: alto };
    espacioAbajo = { x: espacio.x, y: espacio.y + alto, ancho: espacio.ancho, alto: espacio.alto - alto };
  }
