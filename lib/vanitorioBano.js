...
function validarParametros(p) {
  ...
  if (p.nP === 0 && p.nC === 0) throw new Error('El vanitorio debe tener al menos 1 puerta o 1 cajón');
...
function piezasCuerpo(p) {
  const { A, H, P, e, colorInterior, sifon } = p;
...
function distribuirFrentes(p) {
  ...
  if (config === 'solo_puertas') {
    ...
    for (let i = 0; i < nP; i++) {
      piezas.push({
        id: `puerta_${i + 1}`,
        ancho: anchoPuerta, alto: H - 4, espesor: p.gp,
        cantos: ['todos'],
        cantidad: 1,
        color: p.colorExterior, cara: 'exterior',
        posicion: { x: 2 + i * (anchoPuerta + 2), y: 2, z: -p.gp },
        rotacion: 'vertical_frontal',
      });
    }
  ...
  if (config === 'solo_cajones') {
    ...
    for (let i = 0; i < nC; i++) {
      ...
      piezas.push({
        ...
        ancho: A - 4, alto: alturaCajon, espesor: p.gp,
        cantos: ['todos'],
        cantidad: 1,
        color: p.colorExterior, cara: 'exterior',
        posicion: { x: 2, y, z: -p.gp },
        rotacion: 'vertical_frontal',
      });
  ...
  if (config === 'mixto') {
    ...
    for (let i = 0; i < nCajones; i++) {
      ...
      piezas.push({
        id: `frente_cajon_${i + 1}`,
        ancho: A - 4, alto: alturaCajonUnidad, espesor: p.gp,
        cantos: ['todos'],
        cantidad: 1,
        color: p.colorExterior, cara: 'exterior',
        posicion: { x: 2, y, z: -p.gp },
    ...
    for (let i = 0; i < nP; i++) {
      piezas.push({
        ...
        cantidad: 1,
        color: p.colorExterior, cara: 'exterior',
        posicion: { x: 2 + i * (anchoPuerta + 2), y: gap, z: -p.gp },
        rotacion: 'vertical_frontal',
      });
    }
...
function generarHerrajes(p) {
  ...
  if (p.repisas > 0) {
    herrajes.push({ tipo: 'soporte_balda_metalico', cantidad: p.repisas * 4 });
  }

  if (p.nP > 0) {
    const bisagrasPorPuerta = p.H > 650 ? 3 : 2;
  ...
  herrajes.push({ tipo: 'tirador_o_sistema_push', cantidad: p.nP + p.nC });
